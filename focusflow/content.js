...
const TASK_BANNER_ID = "focusflow-task-banner";
...
// ── Voice Typing State ─────────────────────────────────────────
/** @type {HTMLElement|null} Holds the last focused input element */
let ffLastFocusedInput = null;
let ffSpeechRecognition = null;
let ffIsVoiceTyping = false;

document.addEventListener("focusin", (e) => {
    const el = e.target;
    // Track inputs to insert the dictated text into
    if (el && (el.tagName === "INPUT" || el.tagName === "TEXTAREA" || el.isContentEditable)) {
        ffLastFocusedInput = el;
    }
});
...
/**
 * Converts the AI's task extraction output (✅ and ⚠️ items)
 * into a safe, styled checklist HTML layout.
 * @param {string} rawTasks - AI-generated text containing tasks.
 * @returns {string} Safe HTML string with checkable list items.
 */
const parseTasksToHtml = (rawTasks) => {
    ...
    const tasksHtml = lines.map(line => {
        // Check if it's a warning or normal task
        const isWarning = line.startsWith('⚠️') || line.toLowerCase().includes('warning') || line.toLowerCase().includes('deadline');
        const icon = isWarning ? '⚠️' : '✅';
        const cleanedText = line.replace(/^[✅⚠️•\-\*\s]+/, "").replace(/^\d+\.\s*/, "").trim();

        if (!cleanedText) return "";

        const itemClass = isWarning ? 'ff-task-item ff-task-warning' : 'ff-task-item';
        return `
            <li class="${itemClass}">
                <span class="ff-task-icon">${icon}</span>
                <span class="ff-task-text">${escapeHtml(cleanedText)}</span>
            </li>
        `;
    }).join("");

    if (!tasksHtml) {
        return "<ul class=\"ff-tasks-list\"><li>No actionable tasks found on this page.</li></ul>";
    }

    return `<ul class="ff-tasks-list">${tasksHtml}</ul>`;
};

/**
 * Creates and injects the tasks as a persistent left-side panel overlay.
 * Stays open until the user explicitly presses the close button.
 * @param {string} title - The article title.
 * @param {string} tasksMarkup - HTML string of task items.
 */
const injectTaskBanner = (title, tasksMarkup) => {
    // Dismiss existing panel (re-open with fresh content)
    const existing = document.getElementById(TASK_BANNER_ID);
    if (existing) existing.remove();

    const panel = document.createElement("div");
    panel.id = TASK_BANNER_ID;
    panel.className = "ff-task-side-panel";
    panel.setAttribute("role", "dialog");
    panel.setAttribute("aria-label", "FocusFlow AI Task Manager");
    panel.innerHTML = `
    <div class="ff-task-panel-header">
      <div class="ff-task-panel-title">
        <span class="ff-logo">📋 Task Manager</span>
        <span class="ff-badge ff-badge-blue">AI Steps</span>
      </div>
      <button class="ff-task-panel-close" aria-label="Close Task Manager" title="Close">✕</button>
    </div>
    <div class="ff-task-panel-body">
      <h3 class="ff-task-panel-article-title">${escapeHtml(title)}</h3>
      <div class="ff-tasks">${tasksMarkup}</div>
      <p class="ff-caption" style="margin-top:16px;">Generated by Qwen-Coder</p>
    </div>
  `;

    document.body.appendChild(panel);

    // Only closes via X button
    panel.querySelector(".ff-task-panel-close").addEventListener("click", () => {
        panel.classList.add("ff-task-side-panel--closing");
...
const removeSummaryBanner = () => {
    ...
    const taskBanner = document.getElementById(TASK_BANNER_ID);
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (action === "INJECT_TASKS") {
        const { title, rawTasks } = message;
        try {
            const tasksHtml = parseTasksToHtml(rawTasks);
            injectTaskBanner(title, tasksHtml);
            // Focus mode is NOT auto-enabled here — user must toggle it explicitly.
            sendResponse({ success: true });
        } catch (err) {
            console.error("[FocusFlow] Task injection failed:", err);
            sendResponse({ success: false, error: err.message });
        }
        return true;
    }
...
// ─── Google Translate Redirection ──────────────────────────────────────────────
// Replaces DOM injection (which fails CSP on sites like Wikipedia) with a redirect
// to Google's official translate proxy (translate.goog).

const injectGoogleTranslate = () => {
    try {
        const url = window.location.href;

        // If we are already on a translated page (proxy or google translate), revert
        if (url.includes("translate.goog") || url.includes("translate.google.com/translate")) {
            showWidgetToast("Reverting translation...", "info");

            // If it's the official Google Translate page, the 'u' param usually holds the original URL
            const urlObj = new URL(url);
            const originalUrl = urlObj.searchParams.get("u");

            if (originalUrl) {
                window.location.href = originalUrl;
            } else if (urlObj.hostname.endsWith(".translate.goog")) {
                // Fallback for the proxy style
                const originalHostRaw = urlObj.hostname.replace(".translate.goog", "");
                const originalHostname = originalHostRaw.replace(/(?<!-)-(?!-)/g, '.').replace(/--/g, '-');
                const revertUrl = `https://${originalHostname}${urlObj.pathname}${urlObj.search}`;
        ...
        showWidgetToast("Redirecting to Translator...", "info");

        // Use the user's browser language as the target (e.g. 'en-US' -> 'en')
        const browserLang = (navigator.language || 'en').split('-')[0];

        // Official Google Translate redirect URL - most robust for sites like Wikipedia
        const translateUrl = `https://translate.google.com/translate?sl=auto&tl=${browserLang}&u=${encodeURIComponent(url)}`;

        window.location.href = translateUrl;
    } catch (e) {
        console.error("[FocusFlow] Translate failed:", e);
        showWidgetToast("Could not translate this page.", "error");
    }
};
...
const injectFloatingWidget = () => {
    ...
    panel.innerHTML = `
      ...
      <button class="ff-panel-item" data-action="EXTRACT_TASKS_WIDGET" title="Extract actionable tasks and steps">
        <span class="ff-pi-icon">📋</span>
        <span class="ff-pi-text"><span class="ff-pi-name">Page Tasks</span><span class="ff-pi-desc">Get actionable steps</span></span>
      </button>
      ...
      <div class="ff-panel-item ff-panel-item--general" style="border-bottom:none; padding-bottom:4px; align-items:center;">
        <button class="ff-icon-btn${ffIsVoiceTyping ? ' ff-icon-btn--recording' : ''}" data-action="VOICE_TYPING" title="Dictate to text field" aria-label="Voice Typing">
        ...
        <span class="ff-icon-label" style="font-size:12px; font-weight:600; color:#475569; position:relative; top:1px;">Voice input</span>
      </div>
      ...
      <button class="ff-panel-item${isTranslated ? ' ff-panel-item--active' : ''}" data-action="TRANSLATE_PAGE" title="${isTranslated ? 'Revert back to original language' : 'Translate this page'}" translate="no">
        <span class="ff-pi-icon" translate="no">${isTranslated ? '↩️' : '🌍'}</span>
        <span class="ff-pi-text" translate="no"><span class="ff-pi-name" translate="no">${isTranslated ? 'Revert Translation' : 'Translate Page'}</span><span class="ff-pi-desc" translate="no">${isTranslated ? 'Back to original' : 'Change language'}</span></span>
      </button>
    ...
    deleteZone.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg><small>Release to dismiss</small>`;
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        if (Math.abs(dx) > 4 || Math.abs(dy) > 4) {
            dragMoved = true;
            // Show the delete zone immediately once a drag formally starts
    ...
    document.addEventListener("mouseup", (e) => {
        ...
        deleteZone.classList.remove("ff-delete-zone--visible", "ff-delete-zone--danger");
    ...
    panel.addEventListener("click", async (e) => {
        ...
        if (action === "VOICE_TYPING") {
            toggleVoiceTyping();
            // Panel state is updated automatically inside toggleVoiceTyping
        }
        ...
        if (action === "EXTRACT_TASKS_WIDGET") {
            btn.classList.add("ff-panel-item--loading");
            showWidgetToast("Extracting page content…", "info");
            const articleResult = extractArticleContent();
            if (articleResult.error) {
                btn.classList.remove("ff-panel-item--loading");
                showWidgetToast(articleResult.error, "error");
            ...
            showWidgetToast(`Scanning ${articleResult.text.split(/\s+/).length.toLocaleString()} words for tasks…`, "info");
            try {
                const response = await chrome.runtime.sendMessage({
                ...
                btn.classList.remove("ff-panel-item--loading");
                if (response && response.success) {
                    const tasksHtml = parseTasksToHtml(response.tasks);
                    injectTaskBanner(articleResult.title, tasksHtml);
                    showWidgetToast("Task checklist ready!", "success");
                } else {
                    showWidgetToast(response?.error || "Task extraction failed. Check API key.", "error");
                }
            } catch (err) {
                btn.classList.remove("ff-panel-item--loading");
                showWidgetToast("Could not reach background worker.", "error");
    ...
    const refreshPanelState = () => {
        ...
        // Voice Typing button
        const voiceBtn = panel.querySelector('[data-action="VOICE_TYPING"]');
        if (voiceBtn) {
            voiceBtn.classList.toggle("ff-icon-btn--recording", ffIsVoiceTyping);
        }
...
const toggleVoiceTyping = () => {
    if (ffIsVoiceTyping) {
        if (ffSpeechRecognition) ffSpeechRecognition.stop();
        return;
    }

    if (!ffLastFocusedInput) {
        showWidgetToast("Please click an input field first.", "error");
        return;
    }

    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
        showWidgetToast("Voice typing not supported in this browser.", "error");
        return;
    }

    if (!ffSpeechRecognition) {
        ffSpeechRecognition = new SpeechRecognition();
        ffSpeechRecognition.continuous = true;
        ffSpeechRecognition.interimResults = true;
        // Language could be set to user preference, defaulting to en-US
        ffSpeechRecognition.lang = document.documentElement.lang || 'en-US';

        ffSpeechRecognition.onstart = () => {
            ffIsVoiceTyping = true;
            showWidgetToast("Voice typing active. Speak now.", "success");

            // Find the widget panel and refresh its state to show recording indicator
            const panel = document.getElementById(FF_PANEL_ID);
            if (panel) {
                const voiceBtn = panel.querySelector('[data-action="VOICE_TYPING"]');
                if (voiceBtn) voiceBtn.classList.add("ff-icon-btn--recording");
            }
        };

        ffSpeechRecognition.onerror = (event) => {
            console.error("[FocusFlow] Speech recognition error", event.error);
            showWidgetToast("Voice typing error: " + event.error, "error");
            ffIsVoiceTyping = false;

            // Revert UI
            const panel = document.getElementById(FF_PANEL_ID);
            if (panel) {
                const voiceBtn = panel.querySelector('[data-action="VOICE_TYPING"]');
                if (voiceBtn) voiceBtn.classList.remove("ff-icon-btn--recording");
            }
        };

        ffSpeechRecognition.onend = () => {
            ffIsVoiceTyping = false;
            showWidgetToast("Voice typing stopped.", "info");
        ...
        ffSpeechRecognition.onresult = (event) => {
            let finalTranscript = '';
            ...
            if (!ffLastFocusedInput || !finalTranscript) return;

            let textToInsert = finalTranscript;
            // Optionally add a leading space if needed
            if (ffLastFocusedInput.value && !ffLastFocusedInput.value.endsWith(' ')) {
                textToInsert = ' ' + textToInsert;
            }

            ffLastFocusedInput.focus();

            // execCommand is deprecated but still the best way to push to native undo stack
            // and trigger proper Framework input events
            const success = document.execCommand('insertText', false, textToInsert);

            // Fallback for tricky input fields or older browsers
            if (!success) {
                if (typeof ffLastFocusedInput.selectionStart === 'number') {
                    const startPos = ffLastFocusedInput.selectionStart;
                    const endPos = ffLastFocusedInput.selectionEnd;
                    ffLastFocusedInput.value = ffLastFocusedInput.value.substring(0, startPos)
                        + textToInsert
                        + ffLastFocusedInput.value.substring(endPos, ffLastFocusedInput.value.length);
                    ffLastFocusedInput.selectionStart = startPos + textToInsert.length;
                    ffLastFocusedInput.selectionEnd = startPos + textToInsert.length;
                    ffLastFocusedInput.dispatchEvent(new Event('input', { bubbles: true }));
                } else if (ffLastFocusedInput.isContentEditable) {
                    ffLastFocusedInput.innerText += textToInsert;
                    ffLastFocusedInput.dispatchEvent(new Event('input', { bubbles: true }));
                } else {
                    ffLastFocusedInput.value += textToInsert;
                    ffLastFocusedInput.dispatchEvent(new Event('input', { bubbles: true }));
                }
            }
        };
    ...
    try {
        ffSpeechRecognition.start();
    } catch (e) {
        console.error("[FocusFlow] Speech recognition could not start.", e);
        showWidgetToast("Could not start microphone.", "error");
        ffIsVoiceTyping = false;
    }
};
